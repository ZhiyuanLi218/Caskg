"""CaSKG — MCP server for Claude Code.

Provides causal-aware skill retrieval, graph management, and exploration
tools that Claude Code can use to find, load, and manage agent skills.

Launch:
    caskg-server                       # uses CASKG_WORKING_DIR from .env
    caskg-server --workspace ./my_ws   # explicit workspace
"""
...
mcp = FastMCP(
    ...
    instructions=(
        "CaSKG retrieves the most relevant agent skills from a "
...
async def search_skills(
    ...
    workspace: Annotated[str | None, "Workspace directory (defaults to CASKG_WORKING_DIR)"] = None,
) -> str:
...
def skill_retrieval_workflow() -> str:
    """Step-by-step workflow for finding and using skills from the CaSKG graph."""
    return (
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="CaSKG MCP server for Claude Code")
    parser.add_argument(
        "--workspace",
        type=str,
        default=None,
        help="Override CASKG_WORKING_DIR for the session",
    )
    args = parser.parse_args()

    if args.workspace:
        import os
        os.environ["CASKG_WORKING_DIR"] = args.workspace
