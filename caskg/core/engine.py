...
def _chat_api_key(fallback: str | None = None) -> str | None:
    """API key for chat/LLM calls.

    Evaluation uses API_KEY/BASE_URL for chat models and OPENAI_* for
    embeddings. Keep the same split here so graph construction can use an
    embedding proxy without accidentally sending chat requests to it.
    """
    return _env_value("API_KEY") or fallback


def _chat_base_url(fallback: str | None = None) -> str | None:
    configured = _env_value("BASE_URL")
    if configured:
        return _normalize_openai_compat_base_url(configured)
    return fallback
    ...
    try:
        ...
        if provider == "openrouter":
            return LiteLLMService(
                ...
                api_key=_chat_api_key(_resolve_openrouter_api_key()),
                base_url=_chat_base_url(_resolve_openrouter_base_url()),
            )
        ...
        if provider == "openai":
            ...
            return LiteLLMService(
                model=settings.LLM_MODEL,
                api_key=_chat_api_key(api_key),
                base_url=_chat_base_url(optional_base),
            )

        api_key = _secret_value(settings.OPENAI_API_KEY)
        return LiteLLMService(model=model_name, api_key=_chat_api_key(api_key))
...
class SkillGraphRAG(
    ...
    def _resolve_entity_storage_embedding_dim(self) -> int:
        ...
        if workspace_dim != configured_dim:
            logger.info(
                "CaSKG: detected workspace embedding dim "
    ...
    def _bootstrap_prebuilt_workspace(self) -> str:
        ...
        if not source.exists() or not source.is_dir():
            logger.warning(f"CaSKG: prebuilt workspace `{source}` does not exist or is not a directory.")
            return ""

        if target.exists() and any(target.iterdir()):
            logger.info(
                f"CaSKG: workspace `{target}` already has content, skipping bootstrap from `{source}`."
            )
            return ""

        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(source, target, dirs_exist_ok=True)
        logger.info(f"CaSKG: bootstrapped workspace `{target}` from prebuilt graph `{source}`.")
        return str(source)
    ...
    async def _semantic_candidate_scores_for_node(
        ...
        # When semantic linking is disabled, skip the per-node embedding knn
        # query entirely. Running it for every node during indexing fires a
        # burst of embedding calls (one per node, no delay) that trips the
        # provider RPM limit. Lexical/IO candidate ranking covers this path
        # without any API calls; the causal candidate-induction stage computes
        # its own (cached) semantic signal afterwards.
        if not self.config.enable_semantic_linking:
            return []
        source_schema = self._rewrite_node_query_schema(node)
    ...
    async def _link_all_skills(self):
        ...
        try:
            ...
            logger.info(f"CaSKG: linking {len(nodes)} indexed skills.")
            ...
            if edge_map:
                logger.info(f"CaSKG: committing {len(edge_map)} edges.")
    ...
    async def _link_skills_incremental(self, new_node_names: set[str]) -> None:
        ...
        try:
            ...
            if not new_nodes:
                logger.warning(f"CaSKG: incremental link: none of {new_node_names} found after insert.")
                return

            logger.info(
                f"CaSKG: incremental linking {len(new_nodes)} skill(s) against {len(all_nodes)} total."
            )
            ...
            if edge_map:
                logger.info(f"CaSKG: committing {len(edge_map)} incremental edges.")
                await self.state_manager.edge_upsert_policy(
                    self.llm_service,
                    target,
                    list(edge_map.values()),
